...
export default class BreakStatementStruct extends BaseStructInFile {
	constructor(ast, environmentStruct) {
		...
		if (isIdentifierAst(ast.label)) {
			...
			console.error(
				"初始化 BreakStatementStruct, 根据 ast",
				ast,
				"环境结构 environmentStruct",
				environmentStruct,
				"创建 BreakStatementStruct 时, 根据属性 ast.label",
				ast.label,
				"获取标签名, 但是 ast.label 不是 IdentifierAst 也不是 null, 数据类型有误"
			);
			throw new Error(
				"初始化 BreakStatementStruct, 根据 ast, 环境结构 environmentStruct 创建 BreakStatementStruct 时, 根据属性 ast.label 获取标签名, 但是 ast.label 不是 IdentifierAst 也不是 null, 数据类型有误"
			);
		}
