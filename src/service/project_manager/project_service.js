...
/**
 * 校正数据和项目关联
 * 1. 将项目与需求中的时间字段转换为时间戳
 * 2. 根据需求中的 projectIds 重建项目的 requirementIds
 * @returns {Promise<{success: boolean, message: string}>}
 */
export function serviceCorrectData() {
	return Promise.all([dbManager.getAll(STORES.projects), dbManager.getAll(STORES.requirements)]).then(
		([projects, requirements]) => {
			...
			for (let i = 0; i < projects.length; i++) {
				...
				if (proj.createTime !== undefined) {
					const ts = toTimestamp(proj.createTime);
					proj.createTime = ts;
				}
			...
			for (let i = 0; i < requirements.length; i++) {
				...
				const timeFields = ["createTime", "devTime", "testTime", "onlineTime"];
				for (let j = 0; j < timeFields.length; j++) {
					const field = timeFields[j];
					if (req[field] !== undefined) {
						req[field] = toTimestamp(req[field]);
					}
				}
				updatePromises.push(dbManager.put(STORES.requirements, req));
			}

			// 重建项目关联：以需求中的 projectIds 为准
			const projectReqMap = {};
			for (let i = 0; i < projects.length; i++) {
				projectReqMap[projects[i].id] = [];
			}
			for (let i = 0; i < requirements.length; i++) {
				const req = requirements[i];
				const projectIds = req.projectIds || [];
				for (let j = 0; j < projectIds.length; j++) {
					const pid = projectIds[j];
					if (projectReqMap[pid] !== undefined) {
						let alreadyExists = false;
						for (let k = 0; k < projectReqMap[pid].length; k++) {
							if (projectReqMap[pid][k] === req.id) {
								alreadyExists = true;
								break;
							}
						}
						if (!alreadyExists) {
							projectReqMap[pid].push(req.id);
						}
					}
				}
			}

			// 更新所有项目的 requirementIds
			for (let i = 0; i < projects.length; i++) {
				const proj = projects[i];
				proj.requirementIds = projectReqMap[proj.id] || [];
				updatePromises.push(dbManager.put(STORES.projects, proj));
			}
